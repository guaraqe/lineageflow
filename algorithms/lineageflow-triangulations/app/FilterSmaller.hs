...
data Parameter = Parameter
  { parameter_bound
      :: Double
      :% Single
      :? "Maximum value to be accepted by the filtering."
  } deriving Generic
...
operation (Parameter (S b)) (Input (S tri) (S sca)) = (Output (S triF) (S scaF))
  where
    (triF, scaF) =
...
algorithm =
  makeAlgorithm
    "Contact filtering: Keep smaller"
    $(embed "desc/filter-smaller.md")
    operation
